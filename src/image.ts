/**
 * Image upload + compression for the card editor.
 *
 * Handles SVG (as raw text), and JPG/PNG/WebP (resized to 720p, compressed
...
function compress(
  ...
  function search(c: HTMLCanvasElement, m: string, target: number) {
    let lo = 0.1, hi = 0.9, bestURI = '', bestKB = Infinity, bestQ = 0.5;
    for (let i = 0; i < 8; i++) {
      const q = (lo + hi) / 2;
      const uri = c.toDataURL(m, q);
      const kb = dataUriSizeKB(uri);
      if (kb <= target) { bestURI = uri; bestKB = kb; bestQ = q; lo = q; }
      else hi = q;
    }
...
export function setupImageFileHandler(cardItem: HTMLElement): void {
  const fileInput = cardItem.querySelector<HTMLInputElement>('.card-image-file');
  if (!fileInput) return;
  ...
  fileInput.addEventListener('change', (e) => {
    ...
    if (isSVG) {
      reader.onload = (ev) => { svg.value = ev.target!.result as string; };
    ...
    reader.onload = (ev) => {
      ...
      img.onload = () => {
        ...
        if (ow > MAX_WIDTH || oh > MAX_HEIGHT) {
          needsResize = true;
          let nw = ow, nh = oh;
          if (nw > MAX_WIDTH) { nh = (nh * MAX_WIDTH) / nw; nw = MAX_WIDTH; }
          if (nh > MAX_HEIGHT) { nw = (nw * MAX_HEIGHT) / nh; nh = MAX_HEIGHT; }
        ...
        if (!alpha) {
          try {
            const t = canvas.toDataURL('image/webp', 0.8);
            if (t.substring(5, 15) === 'image/webp') { bestMime = 'image/webp'; testURI = t; }
          } catch { /* no webp */ }
        }
        ...
        if (initKB > MAX_SIZE_KB) {
          ...
          dataURI = c.dataURI; sizeKB = c.sizeKB;
          canvas = c.canvas;
          if (needsResize || c.canvas.width !== canvas.width) {
            msgs.push(`Image resized to ${c.canvas.width}×${c.canvas.height} and compressed to ${sizeKB.toFixed(1)} KB (${c.mimeType}, quality ${c.quality}%)`);
          } else {
            msgs.push(`Image compressed to ${sizeKB.toFixed(1)} KB (${c.mimeType}, quality ${c.quality}%)`);
          }
        } else {
          dataURI = testURI; sizeKB = initKB;
